...
def load_imdb(): # 데이터 받아와서 전처리해주는 함수
    from keras.preprocessing.text import Tokenizer
    from keras.datasets import imdb
    max_words = 1000 # 데이터에서 등장 빈도 순위로 몇 번째에 해당하는 단어까지를 사용할 것인지를 의미

    print('Loading data...')
    (x1, y1), (x2, y2) = imdb.load_data(num_words=max_words) # train:test = 25000:25000
    x = np.concatenate((x1, x2)) # x : 정수 인코딩이라는 텍스트 전처리가 끝난 상태의 리뷰 # np.concatenate : 배열 합치는거
    y = np.concatenate((y1, y2)) # y : 긍정(1), 부정(0)
    print(len(x), 'train sequences')

    num_classes = np.max(y) + 1 # y 몇개로 분류되는지 확인
    print(num_classes, 'classes')
...
#     model = Word2Vec.load('word2vec_test_100개.bin')
#
...
# def load_review(df, **param): # x는 리뷰, y는 평점 # df는 word2vec 함수 실행 결과값.
#
#     model = Word2Vec.load('data/word2vec_test.bin') # 학습시킨 모델 불러온다
#     tokenized_review = df['tokenized_review']
#
#     x = get_dataset(list(tokenized_review), model,  param['size']) # x값 어떻게 넘겨주지
#     y = df['score'].to_numpy()
#     return x.astype(float), y
#
# import pandas as pd
# df = pd.read_csv("data/add_token_review.csv")
# param = {'size':100, 'window':4, 'min_count':3}
# x, y = load_review(df, **param)
#
#
# def load_data(dataset_name): # 원하는 데이터셋 입력하면 전처리된 걸로 가져와주는 함수
...
def load_data(dataset_name):
    if dataset_name == 'mnist':
        ...
        df = pd.read_csv('add_token_review_100개.csv')
        param = {'size': 100}
        return load_review(df, **param)
    else:
        print('Not defined for loading', dataset_name)
        exit(0)
