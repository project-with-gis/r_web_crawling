...
def concat_review_csv(input_path):
    ...
    # totalcsv.to_csv('./data/tota_reviews.csv', header=True, index=False)  # data 폴더에 total_reviews.csv라고 지정함
    return totalcsv


def concat_df(*args):
    total_df = pd.concat([*args])
    # total_df.to_csv('./data/toal_reviews.csv', header=True, index=False)
    return total_df
